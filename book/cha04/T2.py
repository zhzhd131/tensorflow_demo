...
with  tf.Session() as sess:
    ...
    for index in range(50):
        ...
        for batch in range(n_batch):
            ...
            sess.run(optimizer,feed_dict={x:batch_x,y:batch_y,keep_prob:0.1})
           #              print(sess.run(loss,feed_dict={x:batch_x,y:batch_y}))  
             
        learning_ra=sess.run(lr)   
        test_acc=sess.run(accracy,feed_dict={x:mnist.test.images,y:mnist.test.labels,keep_prob:0.1})
        #train_acc=sess.run(accracy,feed_dict={x:mnist.train.images,y:mnist.train.labels,keep_prob:0.1}) 

        print(" index:"+str(index)+" test_acc:"+str(test_acc)+" learning_ra:"+str(learning_ra))
